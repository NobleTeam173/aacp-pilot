...
function OverviewTab({ onNavigate }: { onNavigate: (tab: EmpTab) => void }) {
  ...
  if (loading) return <div style={{ color: C.grey, padding: 40, textAlign: 'center' }}>Loading operations overview…</div>;
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 14 }}>
          {[
            ...
            { label: 'Crew Pipeline',      value: pipeline?.totalCompleters ?? 0, accent: undefined },
          ].map(m => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14 }}>
        {([
          { title: 'Submit Operational Requirement', desc: 'Contribute workforce intelligence signals', tab: 'requirements' },
          { title: 'My Signal Submissions', desc: `${signals.length} signal${signals.length !== 1 ? 's' : ''} submitted`, tab: 'submissions' },
          { title: 'Crew Pipeline', desc: `${pipeline?.totalCompleters ?? 0} AACP programme completers`, tab: 'pipeline' },
          { title: 'Sector Overview', desc: 'Aggregated aviation and aerospace intelligence', tab: 'sector' },
      ...
      {signals.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '56px 20px', background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 16 }}>
          ...
          <p style={{ color: C.grey, fontSize: 13, margin: 0 }}>
            Use the Operational Requirements tab to submit your first workforce intelligence signal.
          </p>
        </div>
      ) : (
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '56px 20px', background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 16 }}>
          ...
          <p style={{ color: C.grey, fontSize: 13, margin: 0 }}>
            ...
            Use Operational Requirements to submit your workforce intelligence.
          </p>
        </div>
      ) : (
        <>
...
const TABS: { key: EmpTab; label: string }[] = [
  ...
  { key: 'requirements',  label: 'Operational Requirements'  },
  { key: 'submissions',   label: 'Signal Submissions'        },
  { key: 'pipeline',      label: 'Crew Pipeline'             },
  { key: 'sector',        label: 'Sector Overview'           },
];
...
export function EmployerDashboard() {
  ...
  return (
    <DashboardLayout title="Employer Operations">
      <div style={{ marginBottom: 22 }}>
        <p style={{ color: '#475569', fontSize: 13, margin: 0, lineHeight: 1.5 }}>
          Submit operational requirements, track your workforce intelligence signals, and access crew pipeline and sector intelligence — all powered by the AACP Connector.
        </p>
      </div>
